...
//		outstream.println("Load Time:                  \t"+t);
	...
	private final void buildContigs(){
		...
		for(int i=0; i<THREADS; i++){alpt.add(makeBuildThread(i));}
...
//			System.err.print("F");
	...
	public int extendToRight(final ByteBuilder bb, final int[] rightCounts, final int id){
		...
		while(bb.length()<maxLen){
			...
			for(int i=0; i<4; i++){
				...
				if(count2>=minCountExtend && (!REQUIRE_SAME_COUNT || count2==count)){
					final long y=i;
	...
	private int minCountSeed=1;
	private int minCountExtend=1;
